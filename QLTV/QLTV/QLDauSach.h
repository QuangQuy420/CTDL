...
char dausach_mang[55]= "C:\\Users\\ACER\\Desktop\\QLTV\\QLTV\\dausach_mang.txt";
...
//=========sap xep dau sach=================
void swap(DS_DauSach& x, DS_DauSach& y);
void sapXepDS(DS_DauSach& mang);
...
void NhapDauSach(DS_DauSach &mang)
{
	...
	int s = timMaISBM(mang, p->ISBM); // ktra trung ma ISBM bat nhap lai
	if (p->ISBM == mang.ds[s]->ISBM)
	{
		ThongBao("MA DA TON TAI!");
		p->ISBM= NhapChuoiHoa(14,yNhap,p->ISBM);
	}
	if (p->ISBM.compare("")==0) return;	 // ESC thi return 
	else
	{	
...
void ThemDauSach(DS_DauSach &mang, DauSach p)
{
	...
	mang.ds[mang.sl]->luotmuon = p->luotmuon;	
	mang.ds[mang.sl]->nDMS = p->nDMS;	
	mang.ds[mang.sl]->dms = p->dms;	
	
	mang.sl++; // 
									// 1 2 3.1 3.2  i = 4  
	int i=mang.sl-1; 
	while(i>0 && mang.ds[i]->theloai.compare(mang.ds[i-1]->theloai) <= 0) // so sanh de sap xep theo the loai
	{
		if(mang.ds[i]->theloai.compare(mang.ds[i-1]->theloai) == 0 )  
		{
			if(mang.ds[i]->tensach.compare(mang.ds[i-1]->tensach) >= 0 ) 
			{
				i--;
				continue;
			} // neu trung the loai va ten > thi dung vong lap
		}
...
//	cin>>maDS;
	...
	{
		if (maDS.compare(mang.ds[i]->ISBM) == 0) return i;		
	}
	return 1;
}
...
void hieuChinhDS(DS_DauSach mang)
{
	...
	else 
	{
		...
		gotoxy(0,yNhap+8);	cout << "Nhap nam xuat ban: ";//<<mang.ds[s]->namxb;
		gotoxy(0,yNhap+9);	cout << "Nhap so trang: ";//<<mang.ds[s]->trang;
...
//===Sap xep dau sach theo the loai, neu cung the loai thi sap xep theo ten=== (Selection Sort)
void swap(DS_DauSach& x, DS_DauSach& y)
{
	DS_DauSach temp = x;
	x = y;
...
void sapXepDS(DS_DauSach& mang)
{
	for (int i = 0; i < mang.sl - 1; i++)
		for (int j = i + 1; j < mang.sl; j++)
			if (mang.ds[j]->theloai.compare(mang.ds[i]->theloai) < 0)
				
				swap(mang.ds[j], mang.ds[i]);
			else if (mang.ds[j]->theloai.compare(mang.ds[i]->theloai) == 0)
				if (mang.ds[j]->tensach.compare(mang.ds[i]->tensach) < 0)
					swap(mang.ds[j], mang.ds[i]);
}
...
void xoaDS(DS_DauSach &mang)
{
	...
	{
		if(mang.ds[s]->nDMS == 0 && mang.ds[s]->luotmuon == 0) // chi cho phep xoa dau sach khi dau sach chua co danh muc sach 
		{
			XoaDSDauSach();
			gotoxy(xListDS,yListDS+3);	cout << "|   |      |                        |                  |             |      |     |     |";
			gotoxy(xListDS+2,yListDS+3);	cout << "1";
			...
			{
				delete mang.ds[s];
				for(int i=s+1; i<mang.sl; i++) // cho vòng for chay tu vi tri sau vi tri xoa -> cuoi mang  
				{
			...
			{
				ThongBao("NHAP XAC NHAN SAI");
				InDSDauSach(mang);
				return;
			}		
		}		
		else // dau sach da co danh muc sach 
		{
...
void suaViTriDMS(int ma)
{
	...
	fflush(stdin);
	int ma_sach = NhapSo(16,yNhap, ma_sach);
